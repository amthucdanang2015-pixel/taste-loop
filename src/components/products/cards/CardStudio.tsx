...
import { CARD_SCENES, CARD_FACET_SCENE } from "./scenes";
import { PRIMARY_CTA } from "@/config/brand";
import { ChevronLeft, ChevronRight, Sparkles, ExternalLink } from "lucide-react";

/* ============================================================================
 * Card Studio (D-028 → D-030) — the second product on the Playground contract.
 *
 * LEFT  the shared StyleRail: the same 112 design languages, the same dials.
 * RIGHT the shared ProductStage, running the ENTIRE Cardmaker app as a film:
 *       the editor, live typing, three layouts, the flip, present mode,
 *       export. "Open live demo" hands you the same app, standalone.
 *
 * D-030: nothing product-specific sits below the stage. The content fields,
 * layout picker and export are the PRODUCT's own UI — they live inside the
 * demo, token-themed, exactly where a real user would find them.
 * ==========================================================================*/

const P = productBySlug("cards");
...
export function CardStudio() {
  ...
  const { sceneIdx, setSceneIdx, seek } = useSceneClock(CARD_SCENES, motionPlaying, onLoopEnd);
  ...
  // D-014: a facet edit replays the beat that best demonstrates it. Keyed on
  // replaySeq (not ov) so colour-dragging never remounts the app mid-drag.
  const applyFacet = useCallback((patch: Partial<StyleOverrides>, facet: FacetId) => {
    setOv((o) => ({ ...o, ...patch }));
    seek(CARD_FACET_SCENE[facet]);
    setReplaySeq((s) => s + 1);
  ...
  const scene = CARD_SCENES[sceneIdx];
  // live = the same app, standalone; content is typed THERE (D-030)
  const liveHref = `/playground/cards/live?${encodeStyle(base.slug, ov)}`;
  const replayKey = `${base.slug}:${replaySeq}`;
  ...
  return (
    <>
      <StudioShell
        rail={
          <StyleRail
            intro="The same design-language engine, a different product — a card editor, re-skinned live by every language."
            facets={P.facets}
            active={active}
            setActive={pickStyle}
            ov={ov}
            ...
            resetAll={() => { setOv({}); setReplaySeq((s) => s + 1); }}
        ...
        stage={
          <>
            ...
            {/* the showcase: the ENTIRE product, running as a film */}
            <div className="h-[460px] shrink-0 sm:h-[540px]">
              <ProductStage ds={ds} scene={scene} playing={motionPlaying} devicePin={devicePin} frame={P.frame} chromeUrl={`cardmaker.app · ${ds.slug}`} replayKey={replayKey}>
                {(device) => <CardMaker ds={ds} scene={scene.key} device={device} mode="showcase" />}
              </ProductStage>
            </div>

            {/* timeline + device pin — the shared below-stage row (D-031) */}
            <StageControls scenes={CARD_SCENES} ds={ds} playing={motionPlaying} setPlaying={setPlaying} current={sceneIdx} onSeek={(i) => { setSceneIdx(i); setPlaying(!reduceMotion); }}
