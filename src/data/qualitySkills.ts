 ...
 * model (D-022): nine quality phases mirroring how products are actually built —
 ...
 *
 * Source of truth for the site UI. The publishable Agent Skills mirror lives
 * in /skills at the repo root and is generated by scripts/export-skills.mjs.
 */
...
const CORE_QUALITY_SKILLS: QualitySkill[] = [
  /* ---------------- discovery ---------------- */
  {
    slug: "problem-brief",
    phase: "discovery",
...
import { LAUNCH_QUALITY_SKILLS } from "./qualitySkills.launch";

/**
 * One immutable registry powers directory cards, detail routes, SEO, homepage
 * proof, downloads, and the generated Agent Skills packages.
 */
export const QUALITY_SKILLS: QualitySkill[] = [
  ...CORE_QUALITY_SKILLS,
  ...MORE_SKILLS,
  ...LAUNCH_QUALITY_SKILLS,
].map((skill) => ({
  ...skill,
  smells: skill.smells ?? SMELLS[skill.slug] ?? [],
  verify: skill.verify ?? VERIFY[skill.slug] ?? "",
  essential: ESSENTIALS.includes(skill.slug),
}));
