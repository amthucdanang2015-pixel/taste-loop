...
export default async function Home() {
  ...
  return (
    <>
      <section className="mx-auto grid min-h-[92svh] max-w-7xl items-center gap-10 px-6 pb-16 pt-32 lg:pb-20 lg:pt-36 xl:grid-cols-[0.92fr_1.08fr] xl:gap-14">
        <div className="relative z-10">
          ...
          <div>
            <p className="mt-7 max-w-xl text-pretty text-base leading-relaxed text-white/62 sm:text-lg">
              ...
              evidence. Nam directs a trusted long-running team and a
              coordinated agent system across business, product, design,
              engineering, and QA—then holds the quality gate while real use
              decides what ships next.
            </p>
            <p className="mt-4 flex max-w-xl items-center gap-2 text-sm text-white/48">
              <span className="h-px w-5 bg-signal" />
              Founder-led by {BRAND.founder.name}, backed by more than a decade
              of shared team context.
            </p>
          </div>
          <div>
            <div className="mt-9 flex flex-wrap items-center gap-3">
              <Link href={PRIMARY_CTA.href} data-analytics-location="home-hero" className="inline-flex items-center gap-2 rounded-full bg-loop px-6 py-3 text-sm font-semibold text-ink transition hover:bg-loop/90">
        ...
        <Reveal>
          ...
          <div className="mt-3 grid gap-8 lg:grid-cols-[1fr_0.72fr] lg:items-end">
            <h2 className="max-w-4xl text-3xl font-semibold tracking-[-0.045em] sm:text-5xl">
              AI made production abundant. Choosing what deserves to ship is
              the advantage.
            </h2>
            <p className="text-base leading-relaxed text-white/58">
              Agents can produce many plausible answers in parallel. TasteLoop
              keeps one decision and one quality bar visible: Nam directs, the
              team adds depth, agents accelerate, and evidence decides the next
              loop.
            </p>
          </div>
        </Reveal>
